...
class MapGuideFragment1 : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_guide_map_01, container, false)
    ...
    private fun handleGuideSelection(selectedLayout: ConstraintLayout, guideId: String) {
        ...
        if (selectedGuideLayout == selectedLayout) {
            ...
            callback?.onGuideSelected(null, "MapGuideFragment1")  // 선택 해제 상태 전달
        } else {
            // 이전 선택 항목 해제
            ...
            callback?.onGuideSelected(guideValue, "MapGuideFragment1")
        }
    }
    ...
    private fun saveSelectedGuide(guide: String) {
        ...
        editor.putString("selected_guide_1", guide)
        editor.apply()
    }

    private fun clearSelectedGuide() {
        val editor = sharedPreferences.edit()
        editor.remove("selected_guide_1")
        editor.apply()
    }
